# Bidcoin network Explorer

Real-time block explorer for Bidcoin.
...
```
Bidcoind (RPC) → Indexer Service → Convex (ns-db-1)
                                      ↓
                              Next.js Frontend
...
### 3. Configure Bidcoin RPC

Set the RPC connection in `.env.local`:
```
Bidcoin_RPC_URL=http://95.111.227.14:8332
Bidcoin_RPC_USER=Bidcoin
Bidcoin_RPC_PASS=your-rpc-password
```
...
This syncs blocks from Bidcoind to Convex in real-time.
...
### Indexer
Run on a server with access to Bidcoind RPC:
```bash
CONVEX_URL=... Bidcoin_RPC_URL=... npm run indexer
```
